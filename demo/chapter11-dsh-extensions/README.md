...
这里放的是第 11 章“扩展 dsh 的能力”用到的示例文件。每个子目录对应正文中的一个小节，可以单独使用。

## 11.1 周报 Skill

`11-1-weekly-report/` 用来复现项目级 Skill 生成周报的案例。新建一个空工作区，将下面两项复制进去：
...
`11-2-mcp-filesystem/` 用来复现 MCP 读取工作区外文件的案例。`notes/` 中有一份数据库断连排查笔记，可以把这个目录复制到工作区外的任意位置。
...
`11-3-community-plugin/README.md` 保存本章实测使用的社区插件安装命令：
...
`11-4-tool-decide/` 保存随机选择工具的插件源码和两份配置片段。按照下面的顺序操作：
