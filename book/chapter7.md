...
前几章的任务主要生成文档、表格和演示文稿。本章转向活动海报设计，介绍 dsh 在可视化设计任务中的基本工作流程。此类任务需要同时支持设计文件生成、画布预览和元素修改。Design 插件（deepseek-idesign，由 iPolloWork 发布）能够在会话中增加独立的设计视图，并将模型生成的文件实时渲染到画布中。该插件适用于网站、App 原型和海报等设计任务，其安装方式与第 5 章使用的 ModLens 插件相同。完成本章后，读者将获得一张可继续编辑和导出的社区读书会海报。
...
Design 插件将设计项目保存为一组实际文件，并统一存放在工作区的 `design/` 目录下。dsh 可以继续读取和修改这些文件，设计结果也会保留在工作区中，便于后续调整。该管理方式与第 5 章生成可编辑演示文稿的思路一致。
...
该命令与第 5 章安装 ModLens 时使用相同入口，包名为 `deepseek-idesign`。安装完成后，重新启动 dsh Web 服务，使插件生效：
...
![会话标题栏出现 Design 视图入口](assets/chapter7/7-1-01-design-tab-entry.png)

### 打开 Design 视图

“轨迹”用于查看第 2 章介绍过的原始执行日志。单击 **Design** 后，会话主区域切换为独立画布，左上角显示 iDesign，即 DeepSeek Design 的 Studio 工作台。

![Design 视图打开，Studio 工作台加载](assets/chapter7/7-1-02-design-studio.png){width=78%}
...
![对话中提出活动信息和风格要求，dsh 整理出设计简报](assets/chapter7/7-2-01-brief-input.png){width=66%}
...
![Design 视图中渲染出海报设计稿](assets/chapter7/7-3-01-design-generated.png){width=82%}
...
![海报初稿，暖米色纸底配焦糖棕主色](assets/chapter7/7-3-02-poster-first-draft.png){width=46%}
...
![调整后，主色换成墨绿，点缀色换成淡金](assets/chapter7/7-3-03-design-adjusted.png){width=82%}
...
![打开 Edit 开关后，点选画布元素，元素被圈选并出现浮动工具栏](assets/chapter7/7-3-04-edit-selected-element.png){width=82%}
...
![选中元素后点 Ask AI，对话输入框自动填入包含文件、元素定位和当前文字的修改草稿](assets/chapter7/7-3-05-edit-askai-draft.png){width=74%}
...
![导出后的最终海报成品](assets/chapter7/7-4-01-poster-final-export.png){width=46%}
