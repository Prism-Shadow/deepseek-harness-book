...
dsh 提供了另一种工作方式。我们不要求智能体立即给出答案，而是让它先建立项目、拆分任务，再把相互独立的工作交给多个子代理同时完成。等资料搜集完成后，由主 Agent 核验这些资料并整合报告。

在这个过程中，dsh 不只是一个聊天窗口。智能体可以读取和编辑工作区里的文件、使用网页搜索、维护计划，也可以把工作委派给其他智能体。调研的中间结果会保存在文件中，不会只存在于一段越来越长的对话里。
...
*图 1：使用 dsh 开展深度调研的五个步骤。第三步包含三个并行研究方向，完成后再进入核验和报告生成。*
...
dsh 把被主 Agent 委派出去、独立完成任务的 Agent 称为 subagent，当前中文界面显示为“子代理”。本章后面统一使用这个名称。
...
\begin{center}
\begin{tikzpicture}[every node/.style={align=center, font=\sffamily\footnotesize}]
  \node[dshaccentnode, minimum width=34mm] (main) {主 Agent};
...
这里最关键的词是“同时启动”。主智能体可以在同一轮中委派三个相互独立的任务，dsh 会让它们并行工作，而不是等第一个任务结束后再开始第二个。
...
![任务运行过久时，可以打开子代理记录确认它是否仍在围绕指定方向搜索](assets/chapter3/3-3-03-subagent-search.png)
...
三个子代理分别负责不同方向，主 Agent 则掌握最初的研究问题和全部输出，适合在这里交叉检查材料。本步不再启动新的子代理，而是让主 Agent 读取三份材料，找出来源、冲突、适用范围和信息缺口方面的问题。

在主会话中输入：

```text
请读取“AI代码助手调研/01-研究说明.md”和“研究材料”中的三份文件，检查关键结论是否有来源支持、资料之间是否矛盾、是否遗漏反面材料，以及哪些问题必须通过公司内部数据或试点才能回答。请把结果写入“03-核验记录.md”。
```
...
![主 Agent 完成材料核验后，生成“03-核验记录.md”](assets/chapter3/3-4-01-verification.png)
...
如果缺口仍然属于原来的研究方向，就把补充任务交还给对应的子代理；如果只是材料之间的冲突或适用范围问题，主 Agent 可以直接在核验记录中说明。这里不需要重新拆分整项调研。
...
在这次调研中，我们没有让 dsh 直接回答“是否应该引入 AI 代码助手”，而是完成了一套更接近真实工作的流程：

1. 主 Agent 创建研究说明，明确要解决的问题；
2. 主 Agent 把大问题拆成三个可以并行的任务；
3. 三个子代理同时搜索和整理不同方向的资料；
4. 主 Agent 检查来源、冲突和信息缺口；
5. 主会话根据经过核验的材料生成决策报告。

在这个过程中，读者实际使用了 dsh 的几项核心能力：选择工作区、让 Agent 读写文件、使用网页搜索、委派子代理、查看子代理状态和对话记录，以及在同一个项目中持续修改交付结果。

这也是智能体工作方式与普通问答最重要的区别。普通问答希望模型立即给出一段答案；使用智能体，则是先建立工作过程，让不同智能体和工具共同完成任务，并把中间结果保存在可以检查的文件中。

当调研问题变得更复杂时，可以增加研究方向或进行更多轮核验。但对于第一次使用 Agent 的用户，三个并行子代理、一次由主 Agent 完成的核验和一份最终报告，已经足以建立完整而清晰的使用体验。
