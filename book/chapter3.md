...
模型通常会很快给出一篇结构完整的回答，列出效率、安全、成本等方面的利弊。麻烦在于，我们不知道它查过哪些资料，也分不清哪些结论有证据，哪些只是根据常识作出的推测。

dsh 可以把这项工作拆开来做。先建立项目和研究计划，再把相对独立的任务交给多个子代理，等材料齐备后回到主会话核验并生成报告。研究说明、原始材料和核验记录都会保存在工作区里，不会只留在一段越来越长的对话中。

![使用 dsh 开展深度调研的五个步骤](assets/chapter3/3-1-00-research-flow.svg){.book-technical-figure width=68%}
...
深度调研要先确认究竟解决什么问题，再开始搜索。“要不要引入 AI 代码助手”仍然有很多没有说明的地方。例如，公司是准备直接向整个研发团队推广，还是先选择一个小组试用？公司最关心的是开发速度、代码质量，还是数据安全？研究结果是供研发负责人参考，还是要提交给管理层做采购决策？
...
![dsh 创建研究说明后，回复中列出生成的目录和文件](assets/chapter3/3-1-01-project-created.png){width=68%}
...
从这一步开始，每次都要打开 dsh 写入工作区的文件，确认内容符合要求。后续步骤会继续读取这些文件。
...
![dsh 完成研究计划，明确三条并行任务线及其依赖关系](assets/chapter3/3-2-01-plan-created.png){width=68%}
...
![主会话与三个子代理的文件分工](assets/chapter3/3-3-00-subagent-files.svg){.book-technical-figure width=82%}
...
![任务运行过久时，可以打开子代理记录确认它是否仍在围绕指定方向搜索](assets/chapter3/3-3-03-subagent-search.png){width=68%}
...
![dsh 读取全部研究文件并生成决策报告](assets/chapter3/3-5-01-final-report.png){width=74%}
...
最后打开 `04-决策报告.md`，从推荐方案中挑选几条关键结论，回到研究材料和核验记录中查找依据。如果核验记录中仍有待确认的问题，最终报告也应如实说明。试点范围和评估指标要与前面的材料一致，并写清出现什么情况就停止。发现遗漏时，可以回到主会话继续修改对应文件。
