...
本章接着使用第 1 章已经安装并接通模型的 dsh。开始前，新建一个空目录作为工作区，按照第 1 章的方法在 dsh 中选中该目录，再创建一个标准模式会话。本次实测使用 DeepSeek-V4-Flash，并将推理等级设为 Max。
...
![dsh 创建研究说明后，回复中列出生成的目录和文件](assets/chapter3/3-1-01-project-created.png){width=76%}
...
![dsh 完成研究计划，明确三条并行任务线及其依赖关系](assets/chapter3/3-2-01-plan-created.png){width=76%}
...
![三个子代理同时运行，列表中显示各自的名称、状态、耗时和 token 用量](assets/chapter3/3-3-01-subagents-running.png){width=48%}

dsh 完成委派后会结束主会话的这一轮回复，子代理继续在后台执行。你不必停留在某个子代理页面等待，也不需要反复询问“完成了吗”。任务结束时，列表会改为“当前未运行”，并保留耗时和 token 用量。

![三个子代理完成后，绿色状态点和“当前未运行”表明任务已经结束](assets/chapter3/3-3-02-subagents-completed.png){width=48%}
...
![dsh 根据核验记录重写决策报告，并列出已经修正的问题](assets/chapter3/3-5-01-final-report.png){width=74%}
