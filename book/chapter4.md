...
![本章制作演示文稿的六步流程](assets/chapter4/4-0-01-ppt-workflow.png){width=92%}
...
![在插件配置中展开“视觉引擎（ModLens）”](assets/chapter4/4-1-01-modlens-plugin-settings.png){width=70%}
...
![配置 GLM-5V-Turbo 视觉模型](assets/chapter4/4-1-02-glm-5v-turbo-config.png){width=68%}
...
![dsh 通过 ModLens 读取图片并返回识别结果](assets/chapter4/4-1-03-vision-test.png){width=72%}
...
![4 个子 Agent 在后台分头调研](assets/chapter4/4-2-02-multi-agent-running.png){width=72%}

所有任务结束后，主 Agent 对材料进行合并和去重，并生成 `调研简报.md`。4 份专题报告也保留在工作区中，遇到需要进一步核对的观点时，可以回到对应报告查看原始资料。

![dsh 汇总子 Agent 的调研结果](assets/chapter4/4-2-03-research-complete.png){width=68%}
...
![dsh 调整大纲并保存 outline.md](assets/chapter4/4-3-02-outline-saved.png){width=82%}
...
![dsh 比较视觉方案并推荐“明亮商务”](assets/chapter4/4-3-03-visual-style-options.png){width=84%}
...
![pptx-generator Skill 的文件安装并校验完成](assets/chapter4/4-4-01-skill-installed.png){width=80%}
...
![dsh 使用 ModLens 逐页检查并修改幻灯片](assets/chapter4/4-4-02-modlens-checking.png){width=76%}
...
![最终演示文稿中的 4 张代表性页面](assets/chapter4/4-4-04-slide-previews.png){width=90%}
