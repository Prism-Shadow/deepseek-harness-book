...
将 dsh 接入飞书后，可以直接在聊天窗口中让它读取和创建文档、建立任务和日程、处理审批，并发送消息。

本章以一次虚构的产品版本上线准备为例。我们先让 dsh 读取项目记录并整理要点，再生成一份行动清单。随后，它会建立后续任务和评审日程，把最新进展发送给当前账号，并发起一条请假审批。

## 连接并授权飞书 {#sec-5-1}

本节要建立两条连接。`dsh-lark` 插件负责接收飞书消息并把 dsh 的回答发回原来的聊天；飞书官方 CLI 负责读写文档、创建任务和日程。两者使用同一个飞书自建应用，应用凭据分别保存在 dsh 和飞书 CLI 的凭据存储中。
...
安装完成后重新启动 dsh。打开“设置 → 飞书与 Lark”，填写刚才创建的应用信息，并完成以下设置：

- 域名：选择 `feishu`；
- Workspace：留空，使用 dsh 工作区列表中的第一个工作区；
- Agent Preset：填写 `standard`；
- 单聊策略：选择开放；
- 群聊需要 @机器人：保持开启。

App Secret 只在设置页面中填写，不要放进聊天消息、命令或工作区文件。完成设置后，单击“Save and reconnect”保存并重新连接。
...
![机器人通过飞书返回 dsh 工作区名称](assets/chapter5/5-1-02-lark-connection-test.png){width=72%}
...
本章后续操作均使用当前授权账号，不需要另外准备联系人。

## 读取文档并整理要点 {#sec-5-2}

项目组已经在飞书中建立《2.0 版本上线准备记录》，其中包含上线日期、完成情况、当前风险和后续安排。本节让 dsh 查找并读取这份文档，再整理出便于项目负责人检查的摘要。
...
```text
demo/chapter5-feishu-assistant/5-2-doc/source/2.0版本上线准备记录.md
```

在飞书中新建文档，将文件内容复制进去，并把文档标题设为“2.0 版本上线准备记录”。文档应保存在当前授权账号能够访问的位置。
...
![dsh 读取飞书文档并整理项目进展](assets/chapter5/5-2-02-document-summary.png){width=92%}

## 根据文档生成行动清单 {#sec-5-3}

聊天中的摘要便于快速查看，后续协作还需要一份可以共享和继续编辑的清单。在当前对话中继续发送：
...
dsh 会根据上一轮读取的内容创建新文档，并在回复中给出文档链接，如图 5-5 所示。原来的上线准备记录不会改变。

![dsh 创建 2.0 版本上线行动清单](assets/chapter5/5-3-01-action-list-created.png){width=76%}
...
![飞书任务中记录了负责人和截止时间](assets/chapter5/5-4-02-task-detail.png){width=74%}
...
![数字秘书发送 2.0 版本上线进展消息](assets/chapter5/5-5-01-progress-message.png){width=44%}

## 处理审批流程 {#sec-5-6}

本节使用飞书中名为“请假”的审批模板。当前组织没有该模板时，需要先由管理员在飞书审批管理后台创建并启用。然后向数字秘书发送：
...
dsh 会读取审批定义，填写申请内容并发起审批。回复出现“事假审批已成功发起”，并列出假期类型和申请时间，如图 5-9 所示。

![dsh 通过飞书发起事假审批](assets/chapter5/5-6-01-leave-approval.png){width=60%}
