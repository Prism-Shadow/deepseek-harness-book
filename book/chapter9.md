...
前两章看到的都是 DSH 已经装好的能力。第 8 章拆开讲过，DSH 里能看到的每一样东西，模型适配器、工具注册表、system prompt、agent loop 本身，都是一个个插件，靠 `cordis.patch.yml` 这层配置叠加组装起来。这一章反过来，从用的一侧走到扩展的一侧，动手往这套叠层里加东西。四节各加一样东西，先自己写一个 Skill，再接一个 MCP 服务器进来，然后装一个第三方插件，最后自己写一个几十行的插件塞进配置里。每一节结束时，都会有一个能亲眼看到的、DSH 确实多会了一件事的画面。
...
![没有 Skill 时，DSH 自己组织周报的结构和小标题](assets/chapter9/9-1-01-baseline-no-skill.png)
...
![有 Skill 之后，DSH 先加载 Skill 再按固定格式整理](assets/chapter9/9-1-02-skill-loaded.png)
...
![模型依次调用三个 mcp__notes__ 前缀的工具](assets/chapter9/9-2-01-mcp-tool-calls.png)
...
![基于笔记内容给出的排查建议](assets/chapter9/9-2-02-mcp-result.png)
...
![重启之前，DSH 不知道有这个新工具，只能退回搜网页和翻源码](assets/chapter9/9-3-01-before-restart-no-tool.png)
...
![这次模型直接调用了新装插件带来的 find_dsh_plugin 工具](assets/chapter9/9-3-02-find-tool-call.png)

这次调用记录里出现了 `find_dsh_plugin`，这个工具此前在这台机器上根本不存在，是 `dsh-find-plugin` 这个插件唯一注册的工具。它内部实时查了 GitHub 上带 `dsh-plugin` 标签的仓库。

![带安装命令和安全提醒的搜索结果](assets/chapter9/9-3-03-find-result.png)
...
![设置页的插件列表里能搜到新装的插件，状态是已启用](assets/chapter9/9-3-04-settings-plugin-list.png)
...
![DSH 调用了新写的 pick_one 工具并给出结果](assets/chapter9/9-4-01-pick-one-result.png)
