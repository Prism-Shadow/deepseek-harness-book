...
实测使用独立工作区 `a-share-research`，全程采用标准模式。历史数据整理于 2026 年 8 月 19 日，盘中任务运行于 8 月 20 日。工作区记录了 Python 3.9.6，实测材料没有保留 dsh Web 的准确版本号，正文不补写版本。最终产物分布在 `data/`、`signals/`、`backtest/` 和 `reports/`，所有日期与数字均来自现有文件或运行记录。

行情接口会变化，历史数据也需要持续复核。本章中的趋势判断是基于既定条件的研究推断，不能替代投资决策。
...
中证全指是本章的主要观察对象。中证指数公司 2026 年 4 月 30 日的[指数资料](https://oss-ch.csindex.com.cn/static/html/csindex/public/uploads/indices/detail/files/zh_CN/000985factsheet.pdf)显示，其样本空间包括上交所、深交所和北交所符合条件的股票与存托凭证。北证 50 因本次接口没有返回历史数据而未单列，万得全 A 属于付费数据，也没有用相似代码代替。
...
本次日线主源是腾讯财经。1990 至 1992 年的上证指数异常段使用同花顺数据替换，四个中证系列指数则使用中证指数官网数据补齐或重建；新浪财经只用于近期数据抽查。九份 `data/raw/*.csv` 的字段统一为 `date、open、high、low、close、volume`，`data/combined_close.csv` 保存按日期对齐的收盘价。
...
第一轮取数结束时，dsh 已经声称数据通过完整性检查。我们没有直接进入分析，而是继续追问：

```text
帮我检查一下刚获取的行情数据，看看日期和数值有没有重复、缺失或明显异常，并把检查结果保存下来。先不要分析走势。
```

这条提示漏写了“先报告，不要修改数据”。dsh 在检查后直接修复了文件。处理正式数据时，更稳妥的顺序是先出只读报告，核对问题与来源，再批准修改。本次已经发生写入，只能停止后续修改并重新整理证据：

```text
报告里有些历史原因没有注明来源。请重新整理检查报告，只保留数据能够直接证明的结论；需要解释原因的地方补充可核验来源，否则标成“无法核验”。另外把“发现的问题、采取的修复、修复后的复检结果”分开写清楚，不要再修改数据。
```

检查发现，2015 年 3 月 27 日有八个指数的最低价误填为最高价；中证全指在 2018 年 8 月 3 日至 24 日连续 16 个交易日出现相同收盘值 4666.50，另缺少 2018 年 10 月 23 日和 24 日两条记录。dsh 用同花顺与中证指数官网交叉核对后完成修复。图 9-3 同样是根据 `data/validation_report.md` 整理的核对卡，只保留修复前后的三组主要数字。
...
dsh 随后用界面询问均线类型、统计口径、是否过滤信号和历史区间。图 9-4 记录的是第一项选择。均线类型一旦改变，信号日期也会变化，因此需要由用户明确确认。

![由用户确认采用简单移动平均 SMA](assets/chapter9/9-2-01-rule-choice.png){width=86%}
...
```text
就按刚才定下的主规则实现，先不加入其他辅助指标。生成全部历史信号清单，再画一张最近三年的走势图，标出中证全指、MA20、MA60和信号点，保存到 signals 目录。完成后告诉我信号总数、当前状态和最近五次信号。只描述规则结果，不给投资建议。
```
...
这个结果说明，规则清楚、代码可运行，还不足以证明策略有效。报告中的限制必须和收益数字一起保留：中证全指无法直接交易；回测未计交易成本、滑点和冲击成本；信号确认到执行存在一个交易日的滞后；空仓收益按 0 计算；样本只有一个指数、一组参数和 36 个完整持有段；历史数据经过多源修补，也没有进行样本外检验。历史结果不能外推到未来。

## 定时监控并生成分析报告 {#sec-9-4}

### 先把日报的数据边界守住

前三节已经留下数据、规则与检验结果。接下来把每日动作固化为 `scripts/daily_report.py`，由它更新历史日线、汇总九个指数，再计算中证全指的 MA20、MA60、状态与最近信号。
...
这里的 headless 仅指 Python 脚本不打开浏览器窗口并用无界面后端绘图。这次实测没有验证 dsh Web 的 schedule 提醒、dsh CLI 的 headless profile，也没有配置系统 cron。五轮文件只能证明会话中启动的后台进程连续运行并按时停止。长期无人值守还需要另测调度、失败重试和告警。
...
dsh 先生成 `reports/stock_compare_002156_601899_20260820.md`，补充指标后另存为 `stock_compare_002156_601899_20260820_v2.md`。第二版加入大盘、行业指数、ETF、5/20/60/120 日涨跌、MA20/MA60、MACD、RSI、ATR 和成交量变化。个股日 K 与成交额来自腾讯财经，资金流采用新浪财经口径，行业指数来自中证指数官网，公告和新闻逐条保留链接。所有计算截止 2026 年 8 月 19 日收盘，不含 8 月 20 日盘中数据。

原报告信息较多，Markdown 截图不利于印刷阅读。图 9-11 和图 9-12 将已经核验的字段重新排成两张结果卡，没有补入新的行情事实。第一张先给结论，再列趋势、板块、资金、量能与风险；第二张回到两只股票的日 K 与均线位置。

![两只股票的短期判断及条件](assets/chapter9/9-4-03-stock-analysis-overview.png){width=96%}

结果卡把通富微电概括为“短期震荡”。截至 8 月 19 日，收盘价 63.59 低于 MA20 的 63.71 和 MA60 的 67.05，近 20 日下跌 15.10%，半导体指数同期也下跌 9.47%；业绩预增与 AMD 合作报道提供正面信息，但 ATR 占收盘价 8.4%，波动很高。只有放量站上 MA60 并重新形成金叉，才满足报告中的转强条件。

紫金矿业的结果卡写为“短期上涨趋势”。依据是 20/60 日均线仍处于多头状态，收盘价 32.76 高于 MA60 的 30.02 和 MA120 的 31.96，所属有色指数与 ETF 近 20 日分别上涨 5.50% 和 5.43%，个股近 20 日资金净流入 10.84 亿元。若价格跌破近 20 日低点 30.97 或 MA60，这一判断随即失效。

![两只股票最近约 120 个交易日的日 K 与均线](assets/chapter9/9-4-04-stock-analysis-charts.png){width=96%}

“震荡”和“上涨趋势”都是未来 5 至 20 个交易日的条件推断，表达的是当前证据倾向，不是确定预测。每个新交易日结束后都要重新取数、复算指标，并检查公告来源。到这里，工作区已经能够从市场基准、规则检验一路生成具体标的的研究报告；它仍是研究原型，网页结果卡只呈现已核验产物，不提供交易操作建议。
