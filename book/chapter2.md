...
停止当前的 `dsh web` 进程，再运行一次 `npx -y @deepseek-ai/dsh web`。页面重新打开后，“技术编辑”仍在模式列表中，输入框边框已经恢复为默认颜色。

两个结果对应着不同的保存位置。“技术编辑”的配置已经写入 `$DSH_HOME/.agent-presets/tech-editor/`，dsh 每次启动都会重新读取。主题插件只加载在刚才的进程中，没有写入供下次启动读取的文件。进程结束后，翡翠绿边框也随之消失。
