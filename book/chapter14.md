...
本章通过一个打砖块游戏案例，理解 dsh 如何完成自进化任务。dsh 读取每轮游戏的得分和运行轨迹，修改控制挡板的策略代码，再按固定规则决定保留还是回退。

实验使用 DeepSeek-V4-Flash，最终把固定开发环境中的得分从 303 提高到 833。本次运行保留了 15 次评测的代码、轨迹和结论，最后一轮下降到 739 后，dsh 恢复了第 14 轮策略。

## 从打砖块游戏理解自进化 {#sec-14-1}

### 先看懂这个游戏

实验运行 Atari 打砖块游戏（Breakout）。画面上方是砖块，球在场地中持续反弹，玩家只能左右移动底部挡板。挡板接住球，球就会继续向上撞击砖块；球落到底部则损失一次机会。游戏得分直接反映打掉了多少砖块。

![实际评测环境中的打砖块游戏画面](assets/chapter14/14-1-01-gameplay.png){width=30%}

这张画面只帮助读者理解任务。实验中的策略不读取图像，`environment.py` 从游戏 RAM 中整理出球、挡板和砖块等数值，再交给 `policy.py`。

### 把游戏建模成代码任务

打砖块游戏经过下面的建模后，就能交给 dsh 反复修改和评测。

| 建模部分 | 本次实验的定义 |
|---|---|
| 状态 | 球和挡板坐标、剩余砖块、奖励、连续无奖励步数 |
| 动作 | 空操作、发球、向左、向右 |
| 策略 | `policy.py` 根据当前状态返回一个动作 |
| 评测 | 固定环境中一个完整回合的总得分 |
| 进化对象 | 只修改策略代码和实验笔记 |
| 固定部分 | 环境、评测器、开发种子和实验预算 |

策略还会比较相邻时刻的球坐标，以估算横向和纵向速度。当球向下运动时，它可以预测球到达挡板高度的位置。`evaluate.py` 负责运行完整回合、累计得分并保存轨迹，同一套规则会评测每个候选版本。

本章所说的自进化发生在任务层。模型参数没有训练，变化写进 `policy.py`。dsh 根据评测证据提出假设、修改代码，再选择下一轮从哪个版本继续。

### 两个循环怎样配合

一轮评测内部，策略每个环境步都要读取状态并选择动作。完整回合结束后，dsh 才分析得分和轨迹，进入外层的代码改进过程。

![策略控制游戏与 dsh 改进策略的两层循环](assets/chapter14/14-1-02-evolution-flow.png){width=96%}

外层每轮只处理一个主要问题。dsh 先找到一段具体的失败轨迹，再提出能够由下一次评测证实或否定的假设。修改有效时保留新版本，分数下降或出现明显退化时恢复历史最好策略。一次同时改变落点预测、挡板控制和停止条件，会让结果难以解释。

规则、测量结果和分析结论分别保存。`AGENTS.md` 写允许修改的文件、预算和停止条件；`trials.jsonl` 由评测器逐行追加分数、环境步数、代码哈希和轨迹路径；`evolution-notes.md` 记录 dsh 对轨迹的解释。评测器还会保存每轮策略快照，回退时不依赖模型记忆。

评测器不能跟随候选策略一起变化。本章只允许 dsh 编辑 `policy.py` 和 `evolution-notes.md`。`evaluate.py`、`environment.py`、测试和已生成的记录都受到文件规则保护，并在实验前保存哈希。

本次最多运行 15 个候选版本，每个版本最多使用 27,000 个环境步，累计上限为 405,000 步。达到候选数上限后，dsh 停止修改并恢复历史最好版本。

这个案例只使用 dsh 已有的文件读写和命令执行能力，不需要安装额外插件。网络、Skill、多 Agent 和工作流工具在实验配置中关闭，减少候选策略从实验目录之外取得现成答案的机会。这只能控制本次运行能访问的材料，不能证明模型训练阶段从未接触过类似代码，因此本章把它当作可审计的工程实验。

## 让 dsh 完成 15 轮自进化实验 {#sec-14-2}
...
node ~/deepseek-harness/apps/cli/lib/bin.js \
  ...
  "执行打砖块游戏（Breakout）策略自进化实验。遵守 AGENTS.md，只修改允许的文件；先测基线，每轮提出一个假设并检查轨迹；低分修改必须回退，最多评测 15 次，结束时留下最好策略和完整记录。"
```
...
![15 次打砖块游戏评测的真实得分与历史最好得分](assets/chapter14/14-2-05-score-evolution.png){width=92%}
