...
步骤从请求模型时开始。因此，这次处理有轮次，却没有步骤。日志里会留下
`turn/start` 和 `turn/end`，不会出现 `step/start`。聊天统计行也不会把它算进“轮”数。

只要日志已经写入 `step/start`，这一步就会计数。`ReactLoopAgent` 会在它结束时写入
`step/end`，即使中途失败或被取消也一样。聊天统计行正是根据 `step/end` 计算步数，
所以有些没有显示回复的步骤仍会被计入。在当前轮次结束后，外层的
`while (await this.turn()) {}` 再判断是否还有消息需要处理。没有待处理的消息，循环便停止。

> 深入一点。第 3 章用过的多 Agent 调研，在主 agent 看来也是一种工具调用。
> 工具内部可以再运行一套 agent loop，主 agent 只接收最终返回的结果。
> 工具调用的完整路径会在 11.3 节展开。
...
下一次请求模型时，dsh 只需要带上用户消息、模型的完整回复和工具结果。
dsh 从日志里取出这些内容，按原来的顺序组成消息历史。
这份给模型看的历史叫作 surface。
...
\begin{tikzpicture}[node distance=11mm, every node/.style={align=center, font=\sffamily\footnotesize}]
  ...
  \node[dshseam, below=of log, minimum width=84mm] (surface)
    {筛选出的模型消息\\\footnotesize user/message · assistant/message · tool/result};
...
检查结果可以是放行、拒绝，也可以是请用户确认，只有通过检查的工具随后才能在当前沙箱范围内运行。
...
\begin{tikzpicture}[node distance=10mm and 14mm, every node/.style={align=center, font=\sffamily\footnotesize}]
  ...
  \node[dshaccentnode, below=of gate] (exec) {在沙箱中\\执行工具};
...
dsh 分三步处理过长的上下文，先从最容易缩短的内容下手。

第一步发生在工具刚刚返回结果时。一段纯文本如果超过 50,000 个 UTF-8 字节，
dsh 会把全文保存到单独的 spill（溢出）文件，只把开头、结尾和取回方法交给模型。
这样，一次输出几万行的命令不会立刻挤满下一次请求。模型确实需要中间内容时，
还可以按提示读取原文件。

第二步发生在整段历史接近窗口上限时。dsh 会先找出较早的大型工具结果。
当前默认配置以 8192 个 Unicode 码点为界，保留开头 4096 个和结尾 1024 个码点，
中间换成省略标记。这里改动的是后续模型看到的 surface，原来的 `tool/result` 事件仍在日志中。

如果这样仍然腾不出足够空间，dsh 才会请模型概括较早的消息。默认触发阈值和保留量如下。

$$
\text{触发阈值} = \lfloor 0.8 \times W \rfloor
\qquad
\text{近期保留预算} = \lfloor 0.16 \times W \rfloor
$$

当上下文占用达到窗口的 80% 时，dsh 开始压缩。摘要会替代一段较早的 surface，
最近约 16% 的窗口预算则保留原文。图 \ref{fig-11-5} 用一个缩小的例子表示这次变化。
会话日志中的旧消息不会被删除。
...
上面的数字是本章所用版本的默认值，profile 或模型策略可以修改这些设置。
