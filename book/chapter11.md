...
要查看这些原始记录，可以点击右上角的“Session log”。下载得到的是一个 ZIP 归档，解压后最主要的文件是顶层的 `session.jsonl`。子 Agent 的日志位于 `subagents/<id>/`，会话引用过的图片位于 `media/`。磁盘上的日志默认使用 zstd 压缩，而导出的 `session.jsonl` 已经解码，可以直接用文本编辑器打开。
...
这些事件共同保存了一次任务完整的运行过程，但下一次请求模型时，dsh 并不会把整份日志原样塞进上下文。例如 `turn/start`、`step/end` 和流式生成产生的 `assistant/chunk` 都是运行记录，本身没有必要再次交给模型。

dsh 会从日志中整理出当前的 **surface**。可以把 surface 理解为从完整事件历史中整理出来、用于继续会话的消息状态。它主要包含用户消息、模型的完整回复和工具结果，也可以包含 dsh 或插件注入并记录的运行状态。随后，`deriveMessages()` 再根据这些内容构造下一次模型请求中的消息历史。
...
模型需要使用工具时，会在回复中给出工具名和一组参数。它之所以知道有哪些工具可以使用，是因为 dsh 在模型请求中附上了当前可用的工具表，其中描述了每个工具的名称、用途和参数格式。模型根据这些信息决定是否调用工具，以及传入什么参数。

收到工具调用后，dsh 不会直接执行。它会先记录这次调用，再经过权限规则和 hook 检查。检查可能直接放行，也可能拒绝调用，或者要求用户确认。通过检查以后，工具才会按照自己的执行环境和权限约束运行。
...
模型能够接收的上下文长度有限，但服务商返回的实际 token 用量只有在请求完成后才能得到。如果只依赖这个数字，dsh 就无法在发送请求之前判断内容是否已经接近模型的上下文上限。因此，每次请求发出前，dsh 都会先估算 system prompt、工具描述和消息历史占用的 token 数；如果空间不足，就需要先缩短历史。
...
这种计算只是请求前的近似估计，并不等同于模型实际使用的 tokenizer。对于中文、JSON Schema 等内容，估算值和实际值可能有较大差异。请求成功后，dsh 会记录服务商返回的实际 token 用量。后续请求可以以已经得到的实际用量为基准，只继续估算新增或被替换的部分，从而减少误差。
