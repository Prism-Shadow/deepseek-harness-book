...
![模型与工具在 agent loop 中循环交接](assets/chapter13/13-1-01-agent-loop.svg){.book-technical-figure width=72%}
...
![dsh 从会话日志中整理下一次模型请求所需的历史](assets/chapter13/13-2-01-session-surface.svg){.book-technical-figure width=72%}
...
![dsh 处理一次工具调用的过程](assets/chapter13/13-3-01-tool-call-pipeline.svg){.book-technical-figure width=72%}
...
![一次模型请求中的主要上下文](assets/chapter13/13-4-01-request-context.svg){.book-technical-figure width=72%}
...
![用摘要替代较早历史，同时保留近期消息原文](assets/chapter13/13-4-02-history-compaction.svg){.book-technical-figure width=72%}
