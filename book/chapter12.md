...
Cordis 是一个用于组织 TypeScript 插件的运行时框架。它根据配置加载、更新和卸载插件，为插件提供共享的运行上下文，并管理每个插件实例从启动到退出的状态。正在启动的插件缺少依赖时，Cordis 会让它暂时等待。已经运行的插件不再需要或失去依赖时，Cordis 会卸载它并清理其注册的资源。
