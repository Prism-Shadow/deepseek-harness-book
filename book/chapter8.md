...
初稿先不看 JD，让 dsh 只凭能力档案写，写实是第一位的。它可以把已有事实整理成“做了什么、怎么做的、结果如何”，不能越过原始材料。提示词把这些红线逐条列明。
...
周记里那条独立完成、播放 47 万的视频，可以直接回指个人工作；账号八周从 68450 涨到 83000，只能说明兼职期间的整体变化，材料没有证明这段增长由李小磊一人带来。被组里留用的选题表和直播复盘也要照这个尺度核对。数字要原样保留，个人贡献不能越过材料边界，JD 不看重的内容在优化稿里让位。
...
改完不算完，让 dsh 自己回头查一遍，给每句话找出处。核查结果另存一份
文件，投递稿里不留批注。
...
<!-- 待实测：自查结果与被删句子清单 -->
<!-- 截图 8-3-06（存为 8-3-06-self-check.png）：resume-audit.md 中的逐句证据核查结果 -->
...
自查过关的 `resume-new.md` 还是个 Markdown 文件，投递前要排成一页 PDF。先把简历里留空的联系方式补上。书中演示使用虚构电话和保留域名邮箱，换成自己的材料时再填写真实信息。
...
接着让 dsh 检查当前会话是否具备 PDF 导出能力，再生成文件。缺少所需能力时应当停下来说明原因，不能把 Markdown 文件改个扩展名冒充 PDF。
...
这一节让 dsh 换到桌子对面，当一回面试官。在 `job-hunt` 工作区新建一个普通会话，用一段提示词规定面试规则。进行几轮真实作答后，再拿一份复盘报告。

### 新建模拟面试会话

前面三节留下的文件都在同一个工作区里，换会话不会丢。保持 **Standard mode**，在 `job-hunt` 工作区单击 **New Session**，输入下面的内容。
...
<!-- 待实测：新会话读取三份文件并开始提问的实际过程 -->
<!-- 截图 8-4-01（存为 8-4-01-interview-open.png）：Standard 会话读取材料后的第一个问题 -->
...
<!-- 待实测：多轮模拟面试的实际对话 -->
<!-- 截图 8-4-02（存为 8-4-02-interview-probe.png）：面试官扣住简历数字追问的对话片段 -->
<!-- 截图 8-4-03（存为 8-4-03-interview-rounds.png）：连续几轮问答的完整对话 -->
<!-- 截图 8-4-04（存为 8-4-04-debrief-report.png）：输入“面试结束”后生成并保存的复盘报告 -->

### 对照复盘

把复盘报告里被点名“撑不住”的回答拿去和 8.2 的差距表对照。差距表记录纸面上缺少的证据，复盘报告记录面试时被问住的地方。两边都出现的条目，应当排在秋招补救清单的前面。

四节走完，工作区里应当攒下能力档案、差距表、一版句句有出处的简历，还有一份复盘报告。简历投出去，补救动作排进日程。每场真面试之前，都可以新开一个 Standard 会话，重复这一轮模拟。
