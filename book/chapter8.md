...
全片不使用人物台词，关键信息通过字幕呈现。drama-skills 为各制作阶段提供校验脚本和确认机制，本章将按照这些阶段依次操作。演示项目位于工作区的 `demo/chapter8` 目录，后文路径均以该目录为基准。

## 与 dsh 沟通需求并编写剧本 {#sec-8-1}
...
![dsh 完成剧本：单集卡、因果节拍、分场结构一次给出](assets/chapter8/8-1-02-script-written.png){width=68%}

## 调用 SeedDance 生成分镜 {#sec-8-2}
...
![资产设定完成：角色、造型、场景视图、道具状态链与连续性台账](assets/chapter8/8-2-01-assets-done.png){width=68%}
...
![正式分镜：8 个镜头、景别机位、信息差保护与光线线索](assets/chapter8/8-2-02-storyboard.png){width=68%}
...
![第 1 镜完整提示词包含分段动作、机位、光线、声音和结束状态](assets/chapter8/8-2-03-shot1-video-prompt.png){width=68%}
...
![视频提示词：每镜一段完整可交付提示词，含画风与一致性锚点](assets/chapter8/8-2-04-video-prompts.png){width=68%}
...
![全部 8 个分镜生成完成，audit 对账通过](assets/chapter8/8-2-06-all-shots-report.png){width=68%}
...
![分镜一「雨夜钩子」，冷蓝雨夜里的橘猫](assets/chapter8/8-2-07-frame-hook.png){width=80%}

![分镜五「猫入住小屋」，箱口透出暖光](assets/chapter8/8-2-08-frame-cat-in.png){width=80%}

![分镜七「清晨谢礼」，阳光照亮门垫上的小鱼干](assets/chapter8/8-2-09-frame-fish.png){width=80%}
